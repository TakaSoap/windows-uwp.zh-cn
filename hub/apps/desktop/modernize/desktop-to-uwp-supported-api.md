...
所有桌面应用都支持具有 [DualApiPartition](/uwp/api/Windows.Foundation.Metadata.DualApiPartitionAttribute) 的 API，包括具有包标识的 API 和不具有包标识的 API。 若要确定是否支持要从桌面应用程序调用的 API，请在 [WinRT 命名空间的参考文档](/uwp/api/)中找到你感兴趣的类或成员的参考文章。 然后，查看参考文章中的“属性”  部分是否包含 [DualApiPartition](/uwp/api/Windows.Foundation.Metadata.DualApiPartitionAttribute) 属性。
