...
| attribute                 | 顶点缓冲区元素字段，由一个输入布局指定并且标记有特定的 HLSL 语法。                                                                                |
| 缓冲区对象             | 宽限请参阅 [**D3D11 \_ SUBRESOURCE \_ DATA**](/windows/desktop/api/d3d11/ns-d3d11-d3d11_subresource_data) 和 [**D3D11 \_ buffer \_ DESC**](/windows/desktop/api/d3d11/ns-d3d11-d3d11_buffer_desc) ，并获取一般用途的缓冲区定义。 |
| 帧缓冲区对象 (FBO) | 呈现目标；请参阅具有 [**ID3D11Texture2D**](/windows/desktop/api/d3d11/nn-d3d11-id3d11texture2d) 的 [**ID3D11RenderTargetView**](/windows/desktop/api/d3d11/nn-d3d11-id3d11rendertargetview)。                                       |
| 后台缓冲区               | 具有“后台缓冲区”图面的交换链；请参阅附加了 [**IDXGISurface1**](/windows/desktop/api/dxgi/nn-dxgi-idxgisurface1) 的 [**IDXGISwapChain1**](/windows/desktop/api/dxgi1_2/nn-dxgi1_2-idxgiswapchain1)。                       |
...
-   调用 glBindBuffer 可定义缓冲区的布局，例如总帐 \_ 元素 \_ 数组 \_ 缓冲区。
...
-   使用缓冲区数据元素的配置填充 [**D3D11 \_ SUBRESOURCE \_ 数据**](/windows/desktop/api/d3d11/ns-d3d11-d3d11_subresource_data) 结构。
-   使用缓冲区中单个元素的大小以及缓冲区类型填充 [**D3D11 \_ 缓冲区 \_ DESC**](/windows/desktop/api/d3d11/ns-d3d11-d3d11_buffer_desc) 结构。
-   使用这两个结构调用 [**ID3D11Device1::CreateBuffer**](/windows/desktop/api/d3d11_1/nn-d3d11_1-id3d11device1)。
...
可写入的像素缓冲区或映射（如帧缓冲区）可以创建为 [**ID3D11Texture2D**](/windows/desktop/api/d3d11/nn-d3d11-id3d11texture2d) 对象。 可将这些内容作为资源绑定到 [**ID3D11RenderTargetView**](/windows/desktop/api/d3d11/nn-d3d11-id3d11rendertargetview) 或 [**ID3D11ShaderResourceView**](/windows/desktop/api/d3d11/nn-d3d11-id3d11shaderresourceview)，绘制到其中之后，可以使用关联的交换链进行显示或者传递到着色器。
...
设置统一通常涉及到提供一个 glUniform \* 方法，其中包含 GPU 中的上传位置以及一个指向应用内存中数据的指针。 在 ithe glUniform \* 方法执行后，统一数据在 GPU 内存中，并可由声明该统一的着色器访问。 你应该确保采用这样的方式来打包数据，以便着色器可以基于着色器中的 uniform 声明解释该数据（使用兼容的类型）。
...
Direct3D 将 uniform 数据指定为“常量缓冲区”，与 uniform 一样，它包含提供给各个着色器的常量数据。 与 uniform 缓冲区一样，应采用同样的方法将内存中的常量缓冲区数据打包以使着色器能够解释该数据，这一点非常重要。 使用 DirectXMath 类型 (例如[**XMFLOAT4**](/windows/desktop/api/directxmath/ns-directxmath-xmfloat4)) 而不是平台类型 (如**float \* **或**float \[ 4 \] **) 保证正确的数据元素对齐。
...
通过声明顶点数据元素的格式以及用于每个组件的语义来声明输入布局并将其与顶点着色器关联。 你创建的 D3D11 输入元素 DESC 中描述的顶点元素数据布局 \_ \_ \_ 必须对应于相应结构的布局。 你在此处为包含两个组件的顶点数据创建一个布局：

-   顶点位置坐标在主内存中表示为 XMFLOAT3，它是由 (x, y, z) 坐标的 3 个 32 位浮点值构成的已对齐数组。
-   顶点颜色值表示为 XMFLOAT4，它是由颜色 (RGBA) 的 4 个 32 位浮点值构成的已对齐数组。

为每个组件分配一个语义以及一个格式类型。 然后，将描述传递给 [**ID3D11Device1::CreateInputLayout**](/windows/desktop/api/d3d11/nf-d3d11-id3d11device-createinputlayout)。 如果在呈现方法期间设置输入程序集，则会在调用 [**ID3D11DeviceContext1::IASetInputLayout**](/windows/desktop/api/d3d11/nf-d3d11-id3d11devicecontext-iasetinputlayout) 时使用输入布局。
