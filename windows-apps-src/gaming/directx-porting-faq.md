...
[**QueryPerformanceCounter**](/windows/desktop/api/profileapi/nf-profileapi-queryperformancecounter) 以及 [**QueryPerformanceFrequency**](/windows/desktop/api/profileapi/nf-profileapi-queryperformancefrequency) 仍然是为 UWP 应用实现游戏计时器的最佳方法。
...
仍然使用 RDTSC 指令的游戏需要进行升级。 请参阅[游戏计时和多核处理器](/windows/desktop/DxTechArts/game-timing-and-multicore-processors)。
...
Win32 桌面应用程序仍然使用 DirectSetup，因此如果你还要升级游戏的桌面版本， 请参阅[适用于游戏开发人员的 Direct3D 11 部署](/windows/desktop/direct3darticles/direct3d11-deployment)。
...
-   请阅读[转换到 Direct3D 9](/windows/desktop/direct3d9/converting-to-directx-9)。
-   确保游戏没有固定管道残存 - 请参阅[已弃用的功能](/windows/desktop/direct3d10/d3d10-graphics-programming-guide-api-features-deprecated)。
...
DirectX 10.x 和 DirectX 11 桌面游戏可轻松移植到 UWP。 请参阅 [迁移到 Direct3D 11](/windows/desktop/direct3d11/d3d11-programming-guide-migrating)。

## <a name="how-do-i-choose-the-right-display-device-in-a-multi-monitor-system"></a>如何在多监视器系统中选择正确的显示设备？


用户可以选择用于显示应用的监视器。 通过调用 [**D3D11CreateDevice**](/windows/desktop/api/d3d11/nf-d3d11-d3d11createdevice)（将第一个参数设置为 **nullptr**）让 Windows 提供正确的适配器。 然后，获取该设备的 [**IDXGIDevice interface**](/windows/desktop/api/dxgi/nn-dxgi-idxgidevice)，调用 [**GetAdapter**](/windows/desktop/api/dxgi/nf-dxgi-idxgidevice-getadapter) 并使用 DXGI 适配器来创建交换链。

## <a name="how-do-i-turn-on-antialiasing"></a>如何打开抗锯齿？


创建 Direct3D 设备时会启用抗锯齿（多重采样）。 通过调用[**CheckMultisampleQualityLevels**](/windows/desktop/api/d3d11/nf-d3d11-id3d11device-checkmultisamplequalitylevels)枚举多级多级，然后在调用[**CreateSurface**](/windows/desktop/api/dxgi/nf-dxgi-idxgidevice-createsurface)时在[**DXGI \_ 示例 \_ DESC 结构**](/windows/desktop/api/dxgicommon/ns-dxgicommon-dxgi_sample_desc)中设置多级采样选项。
...
请访问 [Direct3D 11 中的多线程简介](/windows/desktop/direct3d11/overviews-direct3d-11-render-multi-thread-intro)来开始操作。 有关主要差别的列表，请参阅 [Direct3D 版本之间的线程差别](/windows/desktop/direct3d11/overviews-direct3d-11-render-multi-thread-differences)。 请注意，延迟呈现使用设备的*延迟上下文*而不是*即时上下文*。
...
-   [HLSL 编程指南](/windows/desktop/direct3dhlsl/dx-graphics-hlsl-pguide)
-   [Direct3D 10 常见问题](/windows/desktop/DxTechArts/direct3d10-frequently-asked-questions)
...
##  <a name="what-is-the-dxgi_format-equivalent-of-y-surface-format"></a>什么是 DXGI \_ 格式等效于 *y* 表面格式？
