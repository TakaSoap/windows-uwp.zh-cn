...
ms.locfileid: "89163061"
---
# <a name="racing-wheel-and-force-feedback"></a>赛车方向盘和力回馈

本页介绍使用 [RacingWheel][racingwheel] (UWP) 为 Xbox one 赛车轮编程的基础知识，以及通用 Windows 平台的相关 api。
...
赛车方向盘是与真实的赛车驾驶舱外观类似的输入设备。 对于具有汽车或卡车的街机风格和模拟风格的赛车游戏而言，赛车方向盘是完美的输入设备。 赛车方向盘在 Windows 10 和 Xbox One UWP 应用中受 [Windows.Gaming.Input](/uwp/api/windows.gaming.input) 命名空间支持。
...
不同的 Xbox one 赛车提供不同的可选设备功能集和对这些功能的不同级别的支持;此类输入设备之间的这一级别的差异在 [Windows](/uwp/api/windows.gaming.input) 所支持的设备中是唯一的。 并且，你将会遇到的大多设备都将至少支持某些可选功能或者其他功能变体。 因此，单独确定每个连接的赛车方向盘的功能并支持适合游戏的各种功能非常重要。
...
检测与跟踪赛车方向盘的工作原理与检测与跟踪游戏板完全相同，除了使用 [RacingWheel](/uwp/api/windows.gaming.input.racingwheel) 类而不是 [Gamepad](/uwp/api/Windows.Gaming.Input.Gamepad) 类。 有关详细信息，请参阅[游戏板和振动](gamepad-and-vibration.md)。
...
通过调用 [GetCurrentReading](/uwp/api/windows.gaming.input.racingwheel.getcurrentreading#Windows_Gaming_Input_RacingWheel_GetCurrentReading) 轮询赛车方向盘；此函数返回包含赛车方向盘状态的 [RacingWheelReading](/uwp/api/windows.gaming.input.racingwheelreading)。
...
最后，某些赛车方向盘通过方向盘支持力回馈。 你可以读取赛车方向盘的 [WheelMotor](/uwp/api/windows.gaming.input.racingwheel.wheelmotor#Windows_Gaming_Input_RacingWheel_WheelMotor) 属性来确定连接的赛车方向盘是否支持力回馈。 如果 `WheelMotor` 不为 **null**，则支持力回馈，否则不支持力回馈。
...
每个赛车形按钮都 &mdash; 是方向键、 **上一齿轮** 和 **下一齿轮** 按钮的四个方向，另外还有16个按钮， &mdash; 用于指示是否按下 () 或释放 () 。 为了提高效率，按钮读数不以单独的布尔值表示；而是全部打包到一个由 [RacingWheelButtons](/uwp/api/windows.gaming.input.racingwheelbuttons) 枚举表示的单独位域中。
...
从 [RacingWheelReading](/uwp/api/windows.gaming.input.racingwheelreading) 结构的 `Buttons` 属性中读取按钮值。 因为此属性是位域，所以使用按位掩码隔离你感兴趣的按钮值。 设置相应位时按钮为按下（向下）；否则，按钮为释放（向上）。
...
方向盘是必需控件，提供介于 -1.0 和 +1.0 之间的模拟读数。 值 -1.0 对应于方向盘最左的位置；值 +1.0 对应于最右的位置。 从 [RacingWheelReading](/uwp/api/windows.gaming.input.racingwheelreading) 结构的 `Wheel` 属性中读取方向盘的值。
...
油门和刹车为必需控件，每个控件会提供一个介于 0.0（完全释放）和 1.0（完全按下）的模拟读数，用浮点值表示。 油门控件的值通过 [RacingWheelReading](/uwp/api/windows.gaming.input.racingwheelreading) 结构的 `Throttle` 属性读取，刹车控件的值通过 `Brake` 属性来读取。
...
手刹和离合器为可选控件，每个控件会提供一个介于 0.0（完全释放）和 1.0（完全占用）的模拟读数，用浮点值表示。 手刹的值通过 [RacingWheelReading](/uwp/api/windows.gaming.input.racingwheelreading) 结构的 `Handbrake` 属性读取，离合器控件的值通过 `Clutch` 属性来读取。
...
档位是可选控件，会提供一个介于 -1 和 [MaxPatternShifterGear](/uwp/api/windows.gaming.input.racingwheel.maxpatternshiftergear) 之间的数字读数，用带符号的整数值表示。 值 -1 或 0 分别对应于_倒档_ 和_空挡_，递增的正值对应于越来越高的前进挡（最高 **MaxPatternShifterGear** 档，含该档）。 模式移位的值是从[RacingWheelReading](/uwp/api/windows.gaming.input.racingwheelreading)结构的[PatternShifterGear](/uwp/api/windows.gaming.input.racingwheelreading.patternshiftergear)属性中读取的。
...
很多赛车方向盘有力回馈功能，可提供更具沉浸感且更富有挑战性的驾驶体验。 支持力回馈的赛车方向盘通常配备有一个电机，它可以将用力沿单个轴（方向盘旋转轴）应用于方向盘。 Windows 10 和 Xbox one UWP 应用中的 [ForceFeedback](/uwp/api/windows.gaming.input.forcefeedback) 命名空间支持强制反馈。
...
你可以读取赛车方向盘的 [WheelMotor](/uwp/api/windows.gaming.input.racingwheel.wheelmotor#Windows_Gaming_Input_RacingWheel_WheelMotor) 属性来确定连接的赛车方向盘是否支持力回馈。 如果为 null，则不支持强制反馈 `WheelMotor` ; 否则，将支持强制反馈，你可以继续确定汽车的特定反馈功能，如它可以影响的轴。 **null**
...
力回馈效果被加载到回馈设备中，此设备在遇到游戏命令时自主“执行”。 提供了很多基本效果;可以通过实现 [IForceFeedbackEffect](/uwp/api/windows.gaming.input.forcefeedback.iforcefeedbackeffect) 接口的类来创建自定义效果。
