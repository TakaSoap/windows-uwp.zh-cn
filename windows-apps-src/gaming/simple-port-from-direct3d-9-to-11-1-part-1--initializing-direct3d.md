...
在 Direct3D 9 中，我们通过调用 [**IDirect3D9::CreateDevice**](/windows/desktop/api/d3d9/nf-d3d9-idirect3d9-createdevice) 创建了 Direct3D 设备的一个句柄。 我们先获取指向 [**IDirect3D9 interface**](/windows/desktop/api/d3d9helper/nn-d3d9helper-idirect3d9) 的指针，然后指定了控制 Direct3D 设备和交换链配置的很多参数。 执行该操作之前，我们调用了 [**GetDeviceCaps**](/windows/desktop/api/wingdi/nf-wingdi-getdevicecaps) 以确保我们没有要求设备执行无法完成的操作。
...
首先，我们创建设备。 我们会得到设备所支持的功能级别列表 - 该列表将告知我们想知道的有关 GPU 的大部分内容。 而且，我们无需创建接口即可访问 Direct3D。 我们使用 [**D3D11CreateDevice**](/windows/desktop/api/d3d11/nf-d3d11-d3d11createdevice) 核心 API。 这会为我们提供设备句柄以及设备的即时上下文。 设备上下文用来设置管道状态以及生成呈现命令。

创建 Direct3D 11 设备和上下文之后，我们可以利用 COM 指针功能获取最新版本的接口，该指针功能还包括其他功能，因此我们总是推荐使用它。

> **注意**   D3D \_ 功能 \_ 级别 \_ 9 \_ 1 (对应于着色器模型 2.0) 是 Microsoft Store 游戏需要支持的最低级别。 如果你不支持 9 1， (你的游戏的 ARM 包将无法通过认证 \_ ) 。如果你的游戏还包括着色器模型3功能的渲染路径，则 \_ 应 \_ \_ 在阵列中包含 D3D 特征级别 9 \_ 3。
...
> **注意**   这些是 COM 接口，因此第一个响应可能是使用[**QueryInterface**](/windows/desktop/api/unknwn/nf-unknwn-iunknown-queryinterface(q_))。 应该使用 [**Microsoft::WRL::ComPtr**](/cpp/windows/comptr-class) 智能指针。 然后，只需调用 [**As()**](/previous-versions/br230426(v=vs.140)) 方法，从而提供正确接口类型的一个空 COM 指针。
...
既然我们拥有了 DXGI 工厂，那么我们便可以使用它来创建交换链。 下面我们定义交换链参数。 需要指定表面格式;我们将选择 [**DXGI \_ 格式 \_ B8G8R8A8 \_ UNORM**](/windows/desktop/api/dxgiformat/ne-dxgiformat-dxgi_format) ，因为它与 Direct2D 兼容。 我们将关闭显示缩放、多重采样以及立体呈现，因为该示例中没有使用这些功能。 由于我们直接在 CoreWindow 中运行，因此可以将宽度和高度设置为 0 并自动获取全屏值。

> **注意**   对于 UWP 应用，始终将*SDKVersion*参数设置为 D3D11 \_ SDK \_ 版本。
...
为了确保不会经常显示屏幕，我们将帧延迟设置为1，并使用 [**DXGI \_ 交换 \_ 效果 \_ 反向 \_ 顺序**](/windows/desktop/api/dxgi/ne-dxgi-dxgi_swap_effect)。 这不仅能够省电而且还是应用商店的认证要求；我们将在本操作实例的第 2 部分中了解有关呈现到屏幕的更多信息。

> **注意**   您可以使用多线程 (例如， [**ThreadPool**](/uwp/api/Windows.System.Threading)工作项) 在呈现线程被阻止的情况下继续进行其他工作。
