...
-   [下载 Microsoft Visual Studio 2015](https://visualstudio.microsoft.com/vs/)。 Visual Studio 2015 具有一些用于图形编程的工具，例如调试工具。 有关 DirectX 图形和游戏功能及工具的概述，请参阅 [用于 DirectX 游戏开发的 Visual Studio 工具](set-up-visual-studio-for-game-development.md)。
...
-   DirectX 11 和 XAML 应用（通用 Windows）- DirectX 11 和 XAML 应用（通用 Windows）模板创建在使用 DirectX 11 的 XAML 控件内呈现的 UWP 项目。 此模板使用 [**SwapChainPanel**](/uwp/api/Windows.UI.Xaml.Controls.SwapChainPanel)，因此你可以使用 XAML UI 控件。 这可以使添加用户界面元素更容易，但使用 XAML 模板可能会导致 性能降低。
...
-   \*Main 和 \* main-用于管理应用程序资产、更新应用程序状态和呈现帧的方法。
