...
description: 了解如何将 UWP 应用直接连接到 SQL Server 数据库，并使用 System.Data.SqlClient 存储和检索数据。
ms.date: 06/26/2020
ms.topic: article
...
在本指南中，我们将向你介绍一种操作方法。 如果你将 [Northwind](/dotnet/framework/data/adonet/sql/linq/downloading-sample-databases) 示例数据库安装到 SQL Server 实例上，然后使用这些代码段，你最终将获得显示了来自 Northwind 示例数据库的产品的基本 UI。
...
>[!NOTE]
> 本节介绍了一种组织你的数据访问代码的方法。 这不仅仅是为了提供如何使用 [System.Data.SqlClient](/dotnet/api/system.data.sqlclient) 在 SQL Server 数据库中存储和检索数据的示例。 你可以采用对你的应用程序设计最有意义的任何方式组织你的代码。
...
我们将创建一个实现 [INotifyPropertyChanged](/dotnet/api/system.componentmodel.inotifypropertychanged) 事件的类，以便将 XAML UI 中的属性绑定到此类中的属性。
...
创建一个从 Northwind 示例数据库获取产品的方法，然后将这些产品作为 ``Product`` 实例的 [ObservableCollection](/dotnet/api/system.collections.objectmodel.observablecollection-1) 集合返回。
...
### <a name="add-a-basic-user-interface"></a>添加基本用户界面
 ...
 此 XAML 将创建一个 [ListView](/uwp/api/windows.ui.xaml.controls.listview)（用来显示你在上一个代码段中返回的每个产品），并会将 [ListView](/uwp/api/windows.ui.xaml.controls.listview) 中的每个行的属性绑定到我们在 ``Product`` 类中定义的属性。
...
打开 MainPage.xaml.cs 文件，并将代码添加到 ``MainPage`` 类的构造函数，该类可将 [ListView](/uwp/api/windows.ui.xaml.controls.listview) 的 ItemSource 属性设置为 ``Product`` 实例的 [ObservableCollection](/dotnet/api/system.collections.objectmodel.observablecollection-1)。
...
探索 [System.Data.SqlClient](/dotnet/api/system.data.sqlclient) 命名空间以了解 SQL Server 数据库中的数据的其他作用。
