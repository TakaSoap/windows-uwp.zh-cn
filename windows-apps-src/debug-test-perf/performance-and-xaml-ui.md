...
我们已证明可以显著提高性能的步骤是直接移植应用，使之面向 Windows 10（如果尚未如此操作）。 多项 XAML 优化（例如，[{x:Bind}](../xaml-platform/x-bind-markup-extension.md)）仅在 Windows 10 应用中可用。 请参阅[将应用移植到 Windows 10](../porting/index.md) 以及 //build/ 会话[移动到通用 Windows 平台](https://channel9.msdn.com/Events/Build/2015/3-741)。
...
| [ListView 和 GridView UI 优化](optimize-gridview-and-listview.md) | 通过 UI 虚拟化、元素减少和项目的进度更新来改进 [<strong>GridView</strong>](/uwp/api/Windows.UI.Xaml.Controls.GridView) 性能和启动时间。 |
| [ListView 和 GridView 数据虚拟化](listview-and-gridview-data-optimization.md) | 通过数据虚拟化改进 [<strong>GridView</strong>](/uwp/api/Windows.UI.Xaml.Controls.GridView) 性能和启动时间。 |
