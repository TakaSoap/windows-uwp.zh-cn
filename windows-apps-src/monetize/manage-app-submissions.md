...
| applicationCategory           | string  |   为应用指定[类别和/或子类别](../publish/category-and-subcategory-table.md)的字符串。 通过下划线“_”字符将类别和子类别组合为单个字符串，例如 **BooksAndReference_EReader**。      |  
| 定价           |  object  | 包含应用的定价信息的[定价资源](#pricing-object)。        |   
| 可见性           |  string  |  应用的可见性。 这可以是以下值之一： <ul><li>Hidden</li><li>公用</li><li>专用</li><li>NotSet</li></ul>       |   
...
| allowTargetFutureDeviceFamilies           | object   |  键值对字典，其中每个键为 [Windows 10 设备系列](../publish/set-app-pricing-and-availability.md)，而每个值为布尔值，指示是否允许应用面向指定的设备系列。     |    
