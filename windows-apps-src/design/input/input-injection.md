...
使用输入注入，Windows 应用程序可以模拟来自各种输入设备的输入，并将该输入定向到任何位置，包括应用的客户端区域外 (甚至是使用管理员权限运行的应用，如注册表编辑器) 。
...
1. 首先，我们设置 UI (MainPage.xaml)。
    ...
    我们有两个网格区域（一个用于鼠标输入，另一个用于注入式触控输入），每个区域均有四个按钮。
      > [!NOTE] 
      > 必须为网格背景分配一个值 (`Transparent` ，在本例中为) ，否则不检测指针事件。
...
2. 接下来，我们将初始化应用。
    ...
    在此代码段中，我们声明全局对象并声明鼠标输入区域内指针事件的侦听器 ([AddHandler](/uwp/api/windows.ui.xaml.uielement.addhandler))，其在按钮点击事件中可能被标记为已处理。

    [InputInjector](/uwp/api/windows.ui.input.preview.injection.inputinjector) 对象表示用于发送输入数据的虚拟输入设备。

    在 `ContainerInput_PointerPressed` 处理程序中，我们调用触控注入函数。

    在 `ContainerInput_PointerReleased` 处理程序中，我们调用 UninitializeTouchInjection 来关闭 [InputInjector](/uwp/api/windows.ui.input.preview.injection.inputinjector) 对象。
...
3. 下面介绍触控输入注入函数。
    ...
    首先，我们调用 [TryCreate](/uwp/api/windows.ui.input.preview.injection.inputinjector.trycreate) 来实例化 [InputInjector](/uwp/api/windows.ui.input.preview.injection.inputinjector) 对象。

    然后，我们通过 `Default` 的 [InjectedInputVisualizationMode](/uwp/api/windows.ui.input.preview.injection.injectedinputvisualizationmode) 调用 [InitializeTouchInjection](/uwp/api/windows.ui.input.preview.injection.inputinjector.initializetouchinjection)。

    计算注入点之后，我们调用 [InjectedInputTouchInfo](/uwp/api/windows.ui.input.preview.injection.injectedinputtouchinfo) 来初始化要注入的触控点列表（在本示例中，我们创建一个与鼠标输入指针对应的触控点）。

    最后，我们调用 [InjectTouchInput](/uwp/api/windows.ui.input.preview.injection.inputinjector.injecttouchinput) 两次，第一次用于指针向下，第二次用于指针向上。
