...
[**ScrollViewer**](/uwp/api/Windows.UI.Xaml.Controls.ScrollViewer) 控件的平移和滚动设置指示用户在视图内容太多而无法放在视口中时，如何在单个视图内进行导航。 例如，单个视图可以是杂志或书的某一页、计算机的文件夹结构、文档库，也可以是某个相册。

缩放设置适用于光学缩放（受 [**ScrollViewer**](/uwp/api/Windows.UI.Xaml.Controls.ScrollViewer) 控件支持）和 [**Semantic Zoom**](/uwp/api/Windows.UI.Xaml.Controls.SemanticZoom) 控件。 语义式缩放是一种触摸优化技术，它在一个视图内呈现和导航大量的相关数据或内容集。 它使用两个不同的分类模式（或缩放级别）进行工作。 这类似于在单个视图中平移和滚动。 平移和滚动可以与语义式缩放一起使用。

使用应用视图和事件来修改平移/滚动和缩放行为。 这可以比处理指针和手势事件提供更流畅的交互体验。

有关应用视图的详细信息，请参阅[控件、布局和文本](../basics/index.md)。
...
有关个别控件的详细信息，请参阅[控件列表](../controls-and-patterns/index.md)。
...
| [**PointerMoved**](/uwp/api/windows.ui.xaml.uielement.pointermoved)                 | 在屏幕上拖动指针时发生。         |
| [**PointerEntered**](/uwp/api/windows.ui.xaml.uielement.pointerentered)             | 在指针进入元素的点击测试区时发生。 |
| [**PointerExited**](/uwp/api/windows.ui.xaml.uielement.pointerexited)               | 在指针退出元素的点击测试区时发生。  |
| [**PointerCanceled**](/uwp/api/windows.ui.xaml.uielement.pointercanceled)           | 在异常丢失触摸接触时发生。               |
| [**PointerCaptureLost**](/uwp/api/windows.ui.xaml.uielement.pointercapturelost)     | 在另一个元素进行指针捕获时发生。    |
...
| [**ManipulationDelta 事件**](/uwp/api/windows.ui.xaml.uielement.manipulationdelta)                                         | 当输入设备在操作期间更改位置时发生。                                                                      |
| [**ManipulationInertiaStarting 事件**](/uwp/api/windows.ui.xaml.uielement.manipulationinertiastartingevent)                | 在输入设备在操作期间与 [**UIElement**](/uwp/api/Windows.UI.Xaml.UIElement) 对象失去联系和延迟开始时发生。 |
| [**ManipulationCompleted 事件**](/uwp/api/windows.ui.xaml.uielement.manipulationcompleted)                                 | 在 [**UIElement**](/uwp/api/Windows.UI.Xaml.UIElement) 上的操作和延迟完成时发生。                                          |
| [**ManipulationStartingRoutedEventArgs**](/uwp/api/Windows.UI.Xaml.Input.ManipulationStartingRoutedEventArgs)               | 提供 [**ManipulationStarting**](/uwp/api/windows.ui.xaml.uielement.manipulationstarting) 事件的数据。                                         |
| [**ManipulationStartedRoutedEventArgs**](/uwp/api/Windows.UI.Xaml.Input.ManipulationStartedRoutedEventArgs)                 | 提供 [**ManipulationStarted**](/uwp/api/windows.ui.xaml.uielement.manipulationstarted) 事件的数据。                                           |
| [**ManipulationDeltaRoutedEventArgs**](/uwp/api/Windows.UI.Xaml.Input.ManipulationDeltaRoutedEventArgs)                     | 提供 [**ManipulationDelta**](/uwp/api/windows.ui.xaml.uielement.manipulationdelta) 事件的数据。                                               |
| [**ManipulationInertiaStartingRoutedEventArgs**](/uwp/api/Windows.UI.Xaml.Input.ManipulationInertiaStartingRoutedEventArgs) | 提供 [**ManipulationInertiaStarting**](/uwp/api/windows.ui.xaml.uielement.manipulationinertiastarting) 事件的数据。                           |
...
接下来，引发一个或多个 [**ManipulationDelta**](/uwp/api/windows.ui.xaml.uielement.manipulationdelta) 事件。 例如，先触摸屏幕，然后在屏幕上拖动手指。 最后，在完成交互时引发 [**ManipulationCompleted**](/uwp/api/windows.ui.xaml.uielement.manipulationcompleted) 事件。
...
接下来，创建一个名为 `dragTranslation` 的全局 [**TranslateTransform**](/uwp/api/Windows.UI.Xaml.Media.TranslateTransform)，用于转换 [**Rectangle**](/uwp/api/Windows.UI.Xaml.Shapes.Rectangle)。 在 **Rectangle** 上指定一个 [**ManipulationDelta**](/uwp/api/windows.ui.xaml.uielement.manipulationdelta) 事件侦听器，并将 `dragTranslation` 添加到 **Rectangle** 的 [**RenderTransform**](/uwp/api/windows.ui.xaml.uielement.rendertransform)。
...
最后，在 [**ManipulationDelta**](/uwp/api/windows.ui.xaml.uielement.manipulationdelta) 事件处理程序中，使用 [**Delta**](/uwp/api/Windows.UI.Xaml.Shapes.Rectangle) 属性上的 [**TranslateTransform**](/uwp/api/Windows.UI.Xaml.Media.TranslateTransform) 更新 [**Rectangle**](/uwp/api/windows.ui.xaml.input.manipulationdeltaroutedeventargs.delta) 的位置。
