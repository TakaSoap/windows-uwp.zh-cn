...
1. 首先，我们设置 UI (MainPage.xaml)。 
   ...
   UI 包含一个“识别”按钮、一个 [**InkCanvas**](/uwp/api/Windows.UI.Xaml.Controls.InkCanvas) 和一个标准 [**Canvas**](/uwp/api/windows.ui.xaml.controls.canvas)。 当按下“识别”按钮时，将会分析墨迹画布上的所有墨迹笔划，并将对应的形状和文本绘制在标准画布上（如果成功识别）。 原始的笔划墨迹随后将从墨迹画布中删除。
...
2. 在 UI 代码隐藏文件 (MainPage.xaml.cs) 中，添加墨迹和墨迹分析功能所需的命名空间类型引用：
    ...
    - ["UI"。](/uwp/api/windows.ui.input.inking.analysis)
...
5. 在本例中，我们在“识别”按钮的单击事件处理程序中执行墨迹分析。
    - 首先，调用 [**InkCanvas.InkPresenter**](/uwp/api/windows.ui.xaml.controls.inkcanvas.InkPresenter)的 [**StrokeContainer**](/uwp/api/windows.ui.input.inking.inkpresenter.StrokeContainer) 上的 [**GetStrokes**](/uwp/api/windows.ui.input.inking.inkstrokecontainer.GetStrokes)，以获得当前所有墨迹笔划的集合。
...
在上一个部分（[带有墨迹分析的自由格式识别](#free-form-recognition-with-ink-analysis)）中，我们演示了如何使用[墨迹分析 API](/uwp/api/windows.ui.input.inking.analysis) 分析和识别 InkCanvas 区域内的任意墨迹笔划。

在本部分中，我们将演示如何使用 Windows Ink 手写识别引擎（不是墨迹分析）将 [**InkCanvas**](/uwp/api/Windows.UI.Xaml.Controls.InkCanvas) 上的一组笔划转换为文本（基于已安装的默认语言包）。
...
4. 最后，执行基本手写识别。 在本例中，我们使用“识别”按钮的单击事件处理程序来执行手写识别。
   ...
   - [**InkPresenter**](/uwp/api/windows.ui.xaml.controls.inkcanvas.inkpresenter) 将所有笔划墨迹存储在 [**InkStrokeContainer**](/uwp/api/Windows.UI.Input.Inking.InkStrokeContainer) 对象中。 笔划通过 **InkPresenter** 的 [**StrokeContainer**](/uwp/api/windows.ui.input.inking.inkpresenter.strokecontainer) 属性公开，并使用 [**GetStrokes**](/uwp/api/windows.ui.input.inking.inkstrokecontainer.getstrokes) 方法检索。 
    ...
    - 创建一个 [**InkRecognizerContainer**](/uwp/api/Windows.UI.Input.Inking.InkRecognizerContainer) 来管理手写识别过程。
    ...
    - 每个 [**InkRecognitionResult**](/uwp/api/Windows.UI.Input.Inking.InkRecognitionResult) 对象都包含一组文本候选项。 此列表中最顶层的项被识别引擎视为最佳匹配项，后跟其余候选项，以降低置信度。

       我们循环访问每个 [**InkRecognitionResult**](/uwp/api/Windows.UI.Input.Inking.InkRecognitionResult) 并编译候选列表。 然后，将显示候选项，并清除 [**InkStrokeContainer**](/uwp/api/Windows.UI.Input.Inking.InkStrokeContainer) [**)  (**](/uwp/api/Windows.UI.Xaml.Controls.InkCanvas) 。
...
有关 [**InkRecognizer**](/uwp/api/Windows.UI.Input.Inking.InkRecognizer) 支持的语言列表，请参阅 [**InkRecognizer.Name**](/uwp/api/windows.ui.input.inking.inkrecognizer.name) 属性主题。
...
3. 我们使用已安装手写识别器的列表来填充识别器组合框。
   ...
   创建一个 [**InkRecognizerContainer**](/uwp/api/Windows.UI.Input.Inking.InkRecognizerContainer) 来管理手写识别过程。 使用此对象调用 [**GetRecognizers**](/uwp/api/windows.ui.input.inking.inkrecognizercontainer.getrecognizers) 并检索已安装识别器的列表来填充识别器组合框。
...
4. 如果识别器组合框选择发生更改，请更新手写识别器。
   ...
   基于识别器组合框中所选的识别器使用 [**InkRecognizerContainer**](/uwp/api/Windows.UI.Input.Inking.InkRecognizerContainer) 调用 [**SetDefaultRecognizer**](/uwp/api/windows.ui.input.inking.inkrecognizercontainer.setdefaultrecognizer)。
...
1. 这些全球对象（[InkAnalyzer](/uwp/api/windows.ui.input.inking.analysis.inkanalyzer)、[InkStroke](/uwp/api/windows.ui.input.inking.inkstroke)、[InkAnalysisResult](/uwp/api/windows.ui.input.inking.analysis.inkanalysisresult)、[DispatcherTimer](/uwp/api/windows.ui.xaml.dispatchertimer)）用于整个应用。
