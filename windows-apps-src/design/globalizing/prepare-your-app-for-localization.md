...
作为另一个示例，请考虑 "在 {0} () s 分钟内提醒我" 一句。 “minute(s)”适用于英语，但其他语言可能会使用不同的术语。 例如，波兰语使用“minuta”、“minuty”或“minut”，具体取决于上下文。
...
如果要确保在安装后所有语言资源都可用，请为应用包创建一个配置文件，该文件指定在安装过程中需要某些资源， (包括) 的语言资源。 在打包过程中生成应用程序的 .appxbundle 时，将自动启用此优化的安装功能。 有关详细信息，请参阅 [确保在设备上安装资源，而不管设备是否需要](/previous-versions/dn482043(v=vs.140))。

（可选）若要确保安装所有资源 (不只是) 子集，可以在打包应用时禁用 .appxbundle 生成。 但建议不要这样做，因为它会增加应用程序的安装时间。
...
此示例中的格式字符串适用于英语(美国)。 但不适用于德语(德国)，例如，其中的日期和月份以相反顺序显示。 确保转换器了解每个参数的目的，以便它们可以反转格式字符串中的格式项的顺序 (例如，" {1} {0} " ) 适合目标语言。
...
- **本地化工具** 许多本地化工具可用于分析资源文件，只允许翻译人员编辑可翻译字符串。 这种方法减少了翻译人员意外编辑 XML 标记的风险。 但它的缺点是向本地化流程中引入了新的工具和流程。 本地化工具适合具有大量字符串但需要翻译为少数语言的项目。 若要了解详细信息，请参阅[如何使用多语言应用工具包](/previous-versions/windows/apps/jj572370(v=win.10))。
