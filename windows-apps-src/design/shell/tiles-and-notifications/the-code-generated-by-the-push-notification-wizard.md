...
在 \* UploadChannel 方法中，注册设备以接收推送通知。 应用商店将跟踪应用的已安装实例，并提供推送通知通道。 请参阅 [**PushNotificationChannelManager**](/uwp/api/Windows.Networking.PushNotifications.PushNotificationChannelManager)。
...
对于使用 JavaScript 后端的移动服务，服务器端脚本在发生删除、插入、读取或更新操作时运行。 服务器端脚本并不实现这些操作，但是当从客户端调用 Windows Mobile REST API，从而触发这些事件时，这些脚本便会运行。 然后这些脚本自身通过调用 request.execute 或 request.respond 向调用上下文发出响应，从而向这些操作传递控制。 请参阅 [Azure 移动服务 REST API 参考](/previous-versions/azure/reference/jj710108(v=azure.100))。

服务器端脚本中有多种函数可用。 请参阅 [Azure 移动服务中的注册表操作](https://msdn.microsoft.com/library/azure/dn167708.aspx)。 有关所有可用函数的参考，请参阅[移动服务服务器脚本参考](/previous-versions/azure/reference/jj554226(v=azure.100))。
...
Toast 通知易于使用，你可以在为你生成的通道表的 Insert.js 代码中查看示例。 如果你计划使用磁贴通知或锁屏提醒通知，那么你必须为磁贴或锁屏提醒创建 XML 模板，并且必须在模板中指定打包信息的编码。 请参阅[使用磁贴、锁屏提醒和 toast 通知](/previous-versions/windows/apps/hh868259(v=win.10))。
...
应根据 Windows 应用的准则使用推送通知，因为这些通知会占用用户的资源，如果使用过度，可能会分散注意力。 请参阅[推送通知指南和清单](./windows-push-notification-services--wns--overview.md)。

如果你使用推送通知更新动态磁贴，那么你还应该遵循[磁贴和锁屏提醒指南和清单](./creating-tiles.md)中的指南。
