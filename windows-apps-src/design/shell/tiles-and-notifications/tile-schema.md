...
| 属性 | 类型 | 必需 | 说明 |
|---|---|---|---|
| **可视** | [ToastVisual](#tilevisual) | 是 | 描述磁贴通知的视觉部分。 |
...
| **品牌打造** | TileBranding | false | 磁贴应该用来显示应用品牌的形式。 默认情况下，将从默认磁贴继承品牌。 |
| **DisplayName** | 字符串 | false | 在显示此通知时用来覆盖磁贴显示名称的可选字符串。 |
| **参数** | 字符串 | false | 周年更新中的新增功能：当用户从动态磁贴启动应用时通过 LaunchActivatedEventArgs 上的 TileActivatedInfo 属性传递回应用的应用定义数据。 可以通过这些数据了解用户在点击动态磁贴时看到了哪些磁贴通知。 在没有周年更新的设备上，将忽略此功能。 |
...
| **HintAlign** | [AdaptiveTextAlign](#adaptivetextalign) | false | 文本的水平对齐方式。 |
...
| **HintCrop** | [TilePeekImageCrop](#tilepeekimagecrop) | false | 1511 中的新功能：指定要如何裁剪图像。 在 1511 之前的版本中，忽略此功能，速览图像将完全显示，无裁剪。 |
| **AlternateText** | 字符串 | false | 描述图像的替换文本，用于辅助功能。 |
| **AddImageQuery** | bool? | false | 设置为“true”将使 Windows 能够将查询字符串附加到在磁贴通知中提供的图像 URL。 如果你的服务器托管图像并且可以处理查询字符串，则可通过基于查询字符串搜索图像变量，或通过忽略查询字符串并且不带查询字符串按指定方式返回图像，来使用此特性。 此查询字符串指定缩放倍数、对比度设置和语言；例如，通知中提供的“www.website.com/images/hello.png”值将变为“www.website.com/images/hello.png?ms-scale=100&ms-contrast=standard&ms-lang=en-us” |
...
| 属性 | 类型 | 必需 |说明 |
|---|---|---|---|
| **图标** | [TileBasicImage](#tilebasicimage) | 是 | 至少，要支持桌面和移动设备，小磁贴和中等磁贴，请提供分辨率 200x200、PNG 格式、透明且无白色之外其他颜色的方纵横比图像。 有关详细信息，请参阅：[特殊磁贴模板](../tiles-and-notifications/special-tile-templates-catalog.md)。 |
...
| 属性 | 类型 | 必需 |说明 |
|---|---|---|---|
| **图像** | [TileBasicImage](#tilebasicimage) | 是 | 要显示的图像。 |
...
| 属性 | 类型 | 必需 |说明 |
|---|---|---|---|
| **映像** | IList<[TileBasicImage](#tilebasicimage)> | 是 | 将以圆圈形式滚动的图像。 |
...
| 属性 | 类型 | 必需 |说明 |
|---|---|---|---|
| **映像** | IList<[TileBasicImage](#tilebasicimage)> | 是 | 最多可提供 12 个图像（移动设备最多显示 9 个），这些图像将用于幻灯片放映。 如果添加的图像超过 12 个，将引发异常。 |
