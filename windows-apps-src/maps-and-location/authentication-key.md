...
> - Windows 10 版本1703及更早版本：在[中国销售的某些设备](/windows-hardware/customize/desktop/unattend/microsoft-windows-mapcontrol-desktop-chinavariantwin10)上，映射服务不可用

必须先对[通用 Windows 应用](../get-started/universal-application-platform-guide.md)进行身份验证，然后才能使用 MapControl[**命名空间中的**](/uwp/api/Windows.Services.Maps) [**MapControl**](/uwp/api/Windows.UI.Xaml.Controls.Maps.MapControl)和映射服务。 若要对应用进行身份验证，必须指定地图身份验证密钥。 本主题介绍如何从[必应地图开发人员中心](https://www.bingmapsportal.com/)请求地图验证密钥并将其添加到应用。
...
若要在你的通用 Windows 应用中使用 [**MapControl**](/uwp/api/Windows.UI.Xaml.Controls.Maps.MapControl) 和地图服务 ([**Windows.Services.Maps**](/uwp/api/Windows.Services.Maps))，则需要地图验证密钥。 将该密钥添加到地图控件和地图服务对象（如果适用）。

### <a name="to-add-the-key-to-a-map-control"></a>将密钥添加到地图控件

要验证 [**MapControl**](/uwp/api/Windows.UI.Xaml.Controls.Maps.MapControl)，请将 [**MapServiceToken**](/uwp/api/windows.ui.xaml.controls.maps.mapcontrol.mapservicetoken) 属性设置为身份验证密钥值。 你可以在代码中设置该属性，也可以在 XAML 标记中设置它，具体取决于你的偏好。 有关使用 **MapControl** 的详细信息，请参阅[以 2D、3D 和街景视图方式显示地图](display-maps.md)。
...
若要在 [**Windows.Services.Maps**](/uwp/api/Windows.Services.Maps) 命名空间中使用服务，请将 [**ServiceToken**](/uwp/api/windows.services.maps.mapservice.servicetoken) 属性设置为身份验证密钥值。 有关使用地图服务的详细信息，请参阅[显示路线和方向](routes-and-directions.md)和[执行地理编码和反向地理编码](geocoding.md)。
