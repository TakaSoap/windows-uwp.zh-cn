...
ms.locfileid: "89173801"
---
# <a name="choose-a-uwp-version"></a>选择 UWP 版本

每个版本的 Windows 10 都具有适用于 UWP 平台的全新和改进功能。 在 Microsoft Visual Studio 中创建 UWP 应用时，可以选择要面向的版本。 使用 [.NET Standard 2.0](/dotnet/standard/net-standard) 的项目的**最低版本**必须为版本 16299 或更高版本。
...
Windows 10 的每个连续版本中都提供了适用于 UWP 的全新和更改的 API。 有关哪个版本中添加了哪些功能的特定信息，请参阅 [Windows 10 中面向开发人员的新增功能](../whats-new/windows-10-build-19041.md)。

有关枚举所有设备系列及其版本和所有 API 合约及其版本的参考主题，请参阅[设备系列](/uwp/extension-sdks/)和 [API 合约](/uwp/extension-sdks/)。

## <a name="net-api-availability-in-uwp-versions"></a>UWP 版本中的 .NET API 可用性

UWP 支持有限的 .NET API 子集，无论项目的**目标版本**或**最低版本**是什么，都可以使用这些 API。 [此页提供了有关可用类型的详细信息](/dotnet/api/index?view=dotnet-uwp-10.0)。

若要创建可重用的跨平台库，可以在 UWP 中使用 .NET Standard。 [.NET Standard 文档](/dotnet/standard/net-standard)提供了有关哪些版本支持 .NET Standard 的信息。

如果你正在开发桌面应用，请参阅 [.NET Framework 版本和依赖项](/dotnet/framework/migration-guide/versions-and-dependencies)了解有关 .NET Framework 可用性的详细信息。
...
* 使用“目标版本”的值标识用于编译项目的所有参考（合约 WinMD）。 但是这些参考可让你通过对 API 的调用编译代码，这些 API 并不一定存在于你已声明支持的设备上（通过“最低版本”）。 因此，在“最低版本”后引入的任何 API 都需要通过自适应代码调用。 有关自适应代码的详细信息，请参阅[版本自适应代码](../debug-test-perf/version-adaptive-code.md)。