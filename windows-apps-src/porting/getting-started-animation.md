...
如果打开 MainPage.xaml 文件，则会在 **“XAML”** 视图中看到当你在设计器中工作时 Blend 为你添加的 XAML 代码。 特别是要查看 `<Storyboard>` 和 `<Rectangle>` 元素中的代码。 以下代码展示了一个示例。 为简便起见，椭圆形表示省略的不相关代码，并且为提高代码可读性，添加了换行符。
...
可手动编辑此 XAML，或返回到 Blend 以继续在其上执行操作。 Blend 使创建有趣的用户界面变得很有意思，并且使用图形工具设置界面动画的能力可极大加快开发速度。 有关动画的详细信息，请参阅[动画概述](../design/motion/xaml-animation.md)。

**注意**   有关<span class="legacy-term">使用 JavaScript 和 HTML 的 UWP 应用</span>的动画的信息，请参阅对[UI (HTML) 进行动画处理](/previous-versions/windows/apps/hh465165(v=win.10))。

### <a name="next-step"></a>后续步骤

[入门：接下来怎么做？](getting-started-what-next.md)