...
您可以运行 `MakePri.exe help` 或 `MakePri.exe /?` 以查看可用于 MakePri.exe 的命令。 您还可以发出 `MakePri.exe <command> /?` 有关命令的详细信息，并在极少数情况下查看有关 `MakePri.exe <command> <option>` 选项的具体信息。
...
使用索引选项选项 (/io) `new` ，以及、 `resourcepack` 和 `versioned` 来指定提供对资源索引器行为的详细控制的选项。 索引选项默认情况下处于禁用状态。
...
- +/-LinkedFiles (lf) 。 Index (+) 或忽略 (-) 链接文件和文件夹。
...
若要提供主版本号，请使用 [resources@majorVersion](makepri-exe-configuration.md) 配置文件中的属性。
