...
description: 了解如何使用 bingmaps、ms-驱动器到、ms 浏览和 ms 设置 URI 方案从应用启动 Windows Maps 应用。
...
| bingmaps：？ collection = 36.116584 \_ -115.176753 \_ Caesars% 20Palace&lvl = 16 | 显示名为恺撒王宫酒店的某个点（位于拉斯维加斯）的地图并将缩放级别设置为 16。                                                                                                 |
| bingmaps：？ collection = 40.726966 \_ -74.006076 \_ 部分% 255FBusiness        | 显示一个地图，该地图具有一个名为 \_ "拉斯维加斯") 中的 "业务 (" 的点。                                                                                                                               |
...
| 示例 URI | 结果                                                                                                                   |
|--------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------|
| bingmaps：？ collection = 36.116584 \_ -115.176753 \_ Caesars% 20Palace                                                                                                | 搜索拉斯维加斯的恺撒王宫酒店，并在地图上以最佳的地图视图显示结果。                         |
| bingmaps：？ collection = 36.116584 \_ -115.176753 \_ Caesars% 20Palace&lvl = 16                                                                                         | 显示名为恺撒王宫酒店（位于拉斯维加斯）的图钉，缩放级别为 16。                                               |
| bingmaps：？ collection = 36.116584 \_ -115.176753 \_ Caesars% 20Palace ~ 36.113126 \_ -115.175188 \_ % 20Bellagio&lvl = 16&cp = 36.114902 ~-115.176669                   | 显示名为恺撒王宫酒店和百乐宫酒店（均位于拉斯维加斯）的两枚图钉，缩放级别为 16。              |
| bingmaps：？ collection = 40.726966 \_ -74.006076 \_ 虚设% 255FBusiness% 255Fwith% 255FUnderscore                                                                        | 使用带下划线的名为虚设 Business 的图钉显示纽约 \_ \_ \_ 。                                                  |
| bingmaps：？ collection = name。宾馆% 20List ~ 36.116584 \_ -115.176753 \_ Caesars% 20Palace ~ point. 36.113126 \_ -115.175188 \_ % 20Bellagio&lvl = 16&cp = 36.114902 ~-115.176669 | 显示名为酒店列表的列表，以及拉斯维加斯的恺撒王宫酒店和百乐宫酒店的两枚图钉，缩放级别为 16。 |
...
| bingmaps：？ rtp = adr% 20Microsoft% 20Way，% 20Redmond，% 20WA，%2098052 ~ 36.1223 \_ -111.9495 \_ 总计% 20Canyon% 20northern% 20rim | 显示从 1 Microsoft Way, Redmond, WA, 98052 到 Grand Canyon's northern rim 的路线。                                                                |
...
<div class="alert">
<b>注意</b>   省略<b>sty</b>参数将产生与 sty = r 相同的结果。
</div>
<div>
 
</div></td>
...
<td align="left"><p>定义要在地图上绘制的路由的起点和终点，用波形符 () 分隔 <b>~</b> 。 每个经过点可能是通过使用纬度和经度的位置定义的，也可能是通过可选标题或地址标识符定义的。</p>
...
<div class="alert">
<b>注意</b>   如果位置由<b>pos</b>参数值指定，则该标题可用于某个位置。 将显示标题，而不是显示纬度和经度。
</div>
<div>
 
</div></td>
...
| **destination.name** | 目的地的名称 | 示例：destination.name=Redmond, WA | 目标的名称。 你无需编码 **destination.name** 值。 |
