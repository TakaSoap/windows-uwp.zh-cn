...
6.  在 " **市场活动类型**" 下，选择以下选项之一：
    ...
    * **社区广告（免费）**：这些广告将在也创建社区广告市场活动的其他开发人员发布的应用中运行。 在选择此选项之前，必须选择在 "**盈利**  ->  **应用程序广告**" 页中显示社区广告。 有关详细信息，请参阅[关于社区广告](about-community-ads.md)。
...
9.  在**受众**页面中，我们将显示推荐用于市场活动的受众设置。 你可以选择调整此信息：
    ...
    * **年龄范围**：选择所需受众的年龄范围。
    ...
    此部分还显示 **估算范围** 关系图。 根据选定市场中所有支持 Windows 广告的应用用户的百分比，此图显示了通过当前目标选择预计可触及到的受众。
...
11.  在**广告设计**页面中，选择以下选项之一：
    * **自动生成**。 这是默认选项，它允许您从默认模板创建一个广告。 你可以进行选择以自定义广告内容，我们将根据你的选择（进行选择时自动更新）预览广告外观。
        ...
        * 若要进一步自定义广告，请展开 " **自定义 ad 设计"/"查看所有 ad 大小** " 并选择以下任何内容：
            * **背景色**。 在可用选项中选择。
            * **映像**。 选择其中一个可用的图像（来自应用的应用商店一览）。
            * **显示应用分级**。 如果希望显示应用分级，请选中此框。
            * **显示应用免费**。 如果应用在所有选定市场中免费，还可以选择选中此复选框。
            * **调用操作**。 如果你选择**增加应用参与**作为你的市场活动目标，可以将广告的行动号召按钮设置为**打开**、**播放**、**阅读**、**聆听**或**购物**。  
...
12.  如果有 [高级帐户](common-questions.md#how-can-i-increase-the-maximum-monthly-budget-amount-allowed-for-my-ad-campaign)，则可以使用 " **目标 URL** " 框来控制客户单击广告时所发生的情况。
...
13.  最后，单击 " **查看** " 以确认 ad 市场活动的设置，如果是付费广告市场活动，则为其预算和支付信息。 单击 " **确认** "，你的广告通常会在几个小时内开始出现在设备上。

## <a name="review-ad-campaign-performance"></a>查看广告市场活动性能

若要查看市场活动的执行方式，请返回到**广告市场活动**页面。 选择 " **节筛选器** " 可按 **日期**、 **市场活动目标**、 **应用名称**、 **市场活动类型**或 **状态**确定报表中包含的内容的范围。 除了查看有关市场活动的 **印象**、 **单击**、 **转换**和 **花费**的信息外，您还可以使用该报表来 **暂停** 或 **恢复** 市场活动。 有关详细信息，请参阅[广告市场活动报告](/windows/uwp/publish/ad-campaign-report)。
